...
| 035 | `session-035-affected-party-standing-challenge` (PR #53) | Constitutional Affected-Party Standing gating Challenge-raising vertical slice (`db/ddl/015-standing-and-recusal.sql`, `cdp_core.standing_claim` and `cdp_core.standing_recognition_determination` as two separate append-only records, seeded bounded `cdp_standing_recognition_authority` actor; `submit_affected_party_standing_claim`/`recognize`/`narrow`/`deny_standing_claim`; an *optional* `standing_claim_id` gate added to `attest_and_raise_challenge` proving a pending, minimally sufficient claim permits the first Challenge before any recognition determination exists). Explicitly out of scope: Recusal in its entirety, every Standing type other than Affected-Party, automatic Breach Record generation (RFC-CDP-072 is itself E0), the enforcement-projection persistence layer, chained/corrected determinations. | [docs/session-035-affected-party-standing-challenge.md](session-035-affected-party-standing-challenge.md) | PR #53 opened, not yet merged -- CI confirmed passing (run `31146632317`); this row will be corrected with the merge commit once merged, per this file's own discipline of citing `git log --merges` |
