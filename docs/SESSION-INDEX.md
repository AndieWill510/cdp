...
| 034 | `session-034-rfc-033-standing-recognition-clarification` | RFC-CDP-033 clarification (documentation-only): bumps RFC-CDP-033 to Draft v0.7 -- names the required properties of a legitimate Standing recognition role (without naming a specific actor), formalizes provisional affected-party Standing so a minimally sufficient claim is not blocked by pending recognition, defines a five-value recognition outcome vocabulary (`recognized\|narrowed\|deferred\|rejected\|denied`) that pins down what "denial" means for the automatic Breach Record rule, splits the seed Standing Record schema into four append-only records, cites RFC-CDP-078's non-suspension rule, and removes the not-yet-drafted RFC-CDP-075 from `Depends On`. No code. Standing and Recusal remain E0. | [docs/session-034-rfc-033-standing-recognition-clarification.md](session-034-rfc-033-standing-recognition-clarification.md) | PR opened, not yet merged -- this row will be corrected with the actual PR number and merge commit once merged, per this file's own discipline of citing `git log --merges` |
