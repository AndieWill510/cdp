...
| 033 | `session-033-standing-recusal-recon` (PR #51) | Standing and Recusal reconnaissance (documentation-only): reads RFC-CDP-033 v0.6 and the identity/authority closure evidence, answers ten scoping questions, and identifies the missing recognition-authority mechanism as the primary blocker to implementation. No code, no RFC edits beyond what a later session would decide. | [docs/session-033-standing-recusal-recon.md](session-033-standing-recusal-recon.md) | merged (PR #51) |
| 034 | `session-034-rfc-033-standing-recognition-clarification` (PR #52) | RFC-CDP-033 clarification (documentation-only): bumps RFC-CDP-033 to Draft v0.7 -- names the required properties of a legitimate Standing recognition role (without naming a specific actor), formalizes provisional affected-party Standing so a minimally sufficient claim is not blocked by pending recognition, defines a five-value recognition outcome vocabulary (`recognized\|narrowed\|deferred\|rejected\|denied`) that pins down what "denial" means for the automatic Breach Record rule, splits the seed Standing Record schema into four append-only records, cites RFC-CDP-078's non-suspension rule, and removes the not-yet-drafted RFC-CDP-075 from `Depends On`. No code. Standing and Recusal remain E0. | [docs/session-034-rfc-033-standing-recognition-clarification.md](session-034-rfc-033-standing-recognition-clarification.md) | merged (PR #52, merge commit `17d556d`) |
| 035 | `session-035-affected-party-standing-challenge` (PR #53) | Constitutional Affected-Party Standing gating Challenge-raising vertical slice (`db/ddl/015-standing-and-recusal.sql`, `cdp_core.standing_claim` and `cdp_core.standing_recognition_determination` as two separate append-only records, seeded bounded `cdp_standing_recognition_authority` actor; `submit_affected_party_standing_claim`/`recognize`/`deny_standing_claim`; an *optional* `standing_claim_id` gate added to `attest_and_raise_challenge` proving a pending, minimally sufficient claim permits the first Challenge before any recognition determination exists). Explicitly out of scope: Recusal in its entirety, every Standing type other than Affected-Party, automatic Breach Record generation (RFC-CDP-072 is itself E0), the enforcement-projection persistence layer, chained/corrected determinations. Pre-merge review correction: `narrow_standing_claim` and its route were removed -- the determination table has no `outcome_scope` column, so a `narrowed` outcome would have been enforcement-indistinguishable from `recognized`; only `recognized`/`denied` are reachable (see the doc's §2.1). | [docs/session-035-affected-party-standing-challenge.md](session-035-affected-party-standing-challenge.md) | PR #53 opened, not yet merged -- CI confirmed passing on commit `868f191` (run `31146632317`, predates the narrowed-deferral correction); this row will be corrected with the merge commit once merged, per this file's own discipline of citing `git log --merges` |
