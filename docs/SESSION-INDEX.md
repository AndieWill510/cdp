...
| 027 | `session-027-identity-and-attestation` (PR #41) | Identity and Attestation vertical slice (`db/ddl/010-identity-and-attestation.sql`, Actor Registry, Identity Claim, Attestation Record, `attest_and_create_decision` proof path via `POST /attested-decisions`). Explicitly out of scope: Authority, Standing, Legitimize, Repair, real cryptographic verification, OAuth/SSO/password storage. | [docs/session-027-identity-and-attestation.md](session-027-identity-and-attestation.md) | CI passing (run `30677856180`); open, not yet reviewed/merged |
