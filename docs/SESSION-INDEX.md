...
| 027 | (uncommitted) | Identity and Attestation vertical slice (`db/ddl/010-identity-and-attestation.sql`, Actor Registry, Identity Claim, Attestation Record, `attest_and_create_decision` proof path via `POST /attested-decisions`). Explicitly out of scope: Authority, Standing, Legitimize, Repair, real cryptographic verification, OAuth/SSO/password storage. | [docs/session-027-identity-and-attestation.md](session-027-identity-and-attestation.md) | **in progress — implementation drafted and verified locally (Docker Compose); not yet staged, committed, tested in CI, or reviewed** |
