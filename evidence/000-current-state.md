...
|---|---|---|
| Identify (RFC-CDP-030) | Runtime Tested (E3) | DDL: `db/ddl/010-identity-and-attestation.sql` (`cdp_core.actor`, `cdp_core.identity_claim`). Repositories: `cdp/core/repositories/actors.py`, `identity_claims.py`. Service: `register_actor`, `submit_identity_claim`, `recognize_identity_claim`, `deny_identity_claim`, `contest_identity_claim` (`cdp/core/services.py`). Routes: `POST /actors`, `GET /actors/{actor_id}`, `POST /identity-claims`, `GET /identity-claims/{claim_id}`, `POST /identity-claims/{claim_id}/{recognize,deny,contest}` (`cdp/api/identity.py`). Tests: `tests/identify_attest_standing/test_actor_service.py`, `test_identity_claim_service.py` pass against a live local Postgres; `tests/identify_attest_standing/test_identity_attestation_api.py` passes against a live local `uvicorn` + Postgres Docker stack. Not yet E4: these API round-trip results are from a local Docker Compose run, not a confirmed passing GitHub Actions run on this PR's head commit — see the caveat in `README.md#what-an-evidence-level-does-not-claim` about E4 meaning "proven in CI," not "proven locally." This is not authentication/authorization/personhood -- see `db/ddl/010-identity-and-attestation.sql`'s header for the constitutional scope note. |
| Attest (RFC-CDP-031) | Runtime Tested (E3) | DDL: `db/ddl/010-identity-and-attestation.sql` (`cdp_core.attestation_record`). Repository: `cdp/core/repositories/attestations.py`. Service: `attest_and_create_decision` (`cdp/core/services.py`), the proof-path integration with decision creation. Route: `POST /attested-decisions`, `GET /attestations/{attestation_id}` (`cdp/api/identity.py`). Tests: `tests/identify_attest_standing/test_attestation_service.py` (8 cases: happy path, unknown/inactive/mismatched actor, unrecognized/wrong-scope/mismatched-owner claim, transactional rollback) pass against live local Postgres; exercised through the live API by `tests/identify_attest_standing/test_identity_attestation_api.py`. Same E4 caveat as Identify above -- local-only so far, not yet CI-confirmed. "Verified" in this slice means the actor is active and holds a recognized, in-scope identity claim, not cryptographic proof -- see the DDL header for the honest scope statement. |
