...
| Step | Classification | Evidence |
|---|---|---|
| Bearer-token caller binding on actor-asserting mutating routes | Runtime Tested (E3), pending CI confirmation | DDL: `db/ddl/014-caller-authentication.sql` (`cdp_core.actor_bearer_token`, SHA-256 hash only, anti-delete trigger, one-active-token-per-actor partial unique index). Service: `register_actor` additively issues a one-time bearer token (`cdp/core/services.py`); `verify_bearer_token` (standalone, not called from inside any other service function) and `revoke_actor_bearer_token`. Routes: `POST /actors` response gains `bearer_token`; new `POST /actors/{actor_id}/tokens/revoke`; nine existing actor-asserting mutating routes across `cdp/api/identity.py`, `authority.py`, `decisions.py` now require a matching `Authorization: Bearer` header (`POST /identity-claims`, `.../recognize`, `.../deny`, `.../contest`, `/attested-decisions`, `/authority-grants`, `/authority-grants/{id}/revoke`, `.../attested-challenges`, `.../attested-adjudications`, `.../attested-execution-authorizations`, `.../attested-execution-records`). Tests: `tests/migration/test_migration_014_caller_authentication.py` (9 static + 1 Postgres smoke), 8 new cases in `tests/identify_attest_standing/test_actor_service.py`'s `CallerAuthenticationTests`, plus every existing API test in `test_identity_attestation_api.py`/`test_authority_grant_api.py`/`test_universal_attestation_api.py` updated to present the correct token, plus 12 new API cases covering missing/mismatched tokens. All passing against a live Docker Compose stack (288 tests total, zero regressions). Not yet confirmed in CI -- rated E3, not E4, until a GitHub Actions run on this branch's head commit is cited here. This proves the HTTP caller controls a token issued to the actor_id it asserts; it is not OAuth/OIDC, not cryptographic signing (RFC-CDP-031 §4 remains unmet), and has no rotation mechanism -- see `docs/session-032-caller-authentication.md` §1 and §7 for the full boundary statement. |
