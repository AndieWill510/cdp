...
| Standing and Recusal (RFC-CDP-033), general model | Not Implemented (E0) | No evidence available for RFC-CDP-033 as a whole -- Recusal, and every Standing type other than Constitutional Affected-Party, remain unimplemented. RFC-CDP-033 itself was clarified to Draft v0.7 in session 034 (`docs/session-034-rfc-033-standing-recognition-clarification.md`) -- a specification change only. Session 035 implements one bounded slice of this RFC at Integration Tested (E4) -- see the dedicated Standing section below; this row's E0 classification covers everything that slice does not. |
...
## Identity Claim Scope (RFC-CDP-030, richer purpose/scope semantics)

| Step | Classification | Evidence |
|---|---|---|
| Optional registry/decision-class scope on Identity Claims | Integration Tested (E4) | DDL: `db/ddl/013-identity-claim-scope.sql` (additive, nullable `scope_registry_name`/`scope_decision_class_id` columns on `cdp_core.identity_claim`, plus a CHECK constraint forbidding a decision-class scope without a registry scope, added idempotently). Service: `IdentityClaimInput`/`submit_identity_claim` accept the two new optional fields (`cdp/core/services.py`); `_check_claim_recognized_and_scoped` (used by all five `attest_and_*` proof paths) enforces exact-registry-match plus exact-or-wildcard-decision-class matching when a claim sets `scope_registry_name`, mirroring `authority_grant`'s two-level scope model. Route: `POST /identity-claims` accepts the two new optional fields (`cdp/api/identity.py`). Tests: `tests/migration/test_migration_013_identity_claim_scope.py` (7 static + 1 Postgres smoke, including a direct assertion the CHECK constraint fires on INSERT), 4 new cases in `tests/identify_attest_standing/test_identity_claim_service.py`, 3 new cases in `test_attestation_service.py`, 2 new cases in `tests/universal_attestation/test_universal_attestation_service.py` (proving the same check applies to a non-decision-creation proof path), 2 new cases in `test_identity_attestation_api.py`. Confirmed passing in CI job `full-cdp-slice-tests`: run `30730450515`, commit `77f29c9` (this branch's head), 2026-08-02T03:20:23Z, conclusion `success`, alongside the full pre-existing suite with no regressions. Backward-compatible by construction: a claim that omits both new fields (every claim submitted before this migration) behaves identically to its pre-session-030 behavior -- see `docs/session-030-identity-claim-scope.md` §1. |

## Caller Authentication (RFC-CDP-030 §6 / RFC-CDP-031 §7)
...
## Standing (RFC-CDP-033), Affected-Party / Challenge slice (session 035)

| Step | Classification | Evidence |
|---|---|---|
| Constitutional Affected-Party Standing gating Challenge-raising | Integration Tested (E4) | DDL: `db/ddl/015-standing-and-recusal.sql` (`cdp_core.standing_claim`, `cdp_core.standing_recognition_determination` as a separate append-only record, both immutable via forbid-delete *and* forbid-update triggers; minimal sufficiency per RFC-CDP-033 §11.4 enforced by CHECK constraints; seeded bounded `cdp_standing_recognition_authority` actor). Repository: `cdp/core/repositories/standing.py`. Service: `submit_affected_party_standing_claim`, `recognize_standing_claim`, `deny_standing_claim`, and an *optional* Standing gate added to `attest_and_raise_challenge` via a new `standing_claim_id` parameter on `AttestedChallengeInput` (`cdp/core/services.py`) -- optional, not mandatory, since RFC-CDP-033 §6 names multiple bases for Challenge standing and this slice implements only one; see the service function's own docstring. Routes: `POST /standing-claims`, `GET /standing-claims/{claim_id}`, `POST /standing-claims/{claim_id}/{recognize,deny}` (`cdp/api/standing.py`); `POST /decisions/{registry_name}/{decision_id}/attested-challenges` accepts the same optional `standing_claim_id` (`cdp/api/decisions.py`). Only two of RFC-CDP-033 §11.8's five recognition outcomes are reachable -- `recognized`, `denied` -- after a pre-merge review correction removed `narrow_standing_claim` and its route: this table has no `outcome_scope` column (RFC-CDP-033 §9.2) to record what a narrowing narrows to, so a `narrowed` determination would have been enforcement-indistinguishable from `recognized` while still asserting something the system could not describe; `narrowed` remains seeded in the vocabulary only, reserved for a future session. Tests: `tests/migration/test_migration_015_standing_and_recusal.py` (16 cases: 15 static + 1 Postgres smoke, including a direct assertion that `narrowed` is seeded in the vocabulary but rejected by the determination table's own CHECK constraint), `tests/standing/test_standing_claim_service.py` (21 cases, including the load-bearing proof that a pending, minimally sufficient claim with no determination yet permits the first Challenge, that a `recognized` determination also permits it while only `denied` blocks, that `narrow_standing_claim` does not exist, and that a direct `narrowed` insert is rejected by the database), `tests/standing/test_standing_claim_api.py` (15 cases, including a direct assertion that `POST .../narrow` returns 404 as an unregistered route). Confirmed passing in CI job `full-cdp-slice-tests`: run `31146632317`, PR #53 head commit `868f191161f928e1cc3c4896737d5853a1d9b2be`, 2026-08-07T04:11:29Z, conclusion `success` (the implementation this run tested predates the narrowed-deferral correction below, but the schema, gate mechanics, and optional-parameter behavior it verified are unchanged by that correction), alongside the full pre-existing suite (`pr-guard` and `full-cdp-slice-tests` both `pass`) with no regressions. This is not Recusal (unimplemented in its entirety), not any Standing type other than Constitutional Affected-Party, not automatic Breach Record generation on `denied` (RFC-CDP-072 remains E0), and not the enforcement-projection half of RFC-CDP-033 §12's two-layer persistence model -- see `docs/session-035-affected-party-standing-challenge.md` §2.1 for the narrowed-deferral correction and the full boundary statement. |
