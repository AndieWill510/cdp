...
| Step | Classification | Evidence |
|---|---|---|
| Constitutional Affected-Party Standing gating Challenge-raising | Integration Tested (E4) | DDL: `db/ddl/015-standing-and-recusal.sql` (`cdp_core.standing_claim`, `cdp_core.standing_recognition_determination` as a separate append-only record, both immutable via forbid-delete *and* forbid-update triggers; minimal sufficiency per RFC-CDP-033 §11.4 enforced by CHECK constraints; seeded bounded `cdp_standing_recognition_authority` actor). Repository: `cdp/core/repositories/standing.py`. Service: `submit_affected_party_standing_claim`, `recognize_standing_claim`, `narrow_standing_claim`, `deny_standing_claim`, and an *optional* Standing gate added to `attest_and_raise_challenge` via a new `standing_claim_id` parameter on `AttestedChallengeInput` (`cdp/core/services.py`) -- optional, not mandatory, since RFC-CDP-033 §6 names multiple bases for Challenge standing and this slice implements only one; see the service function's own docstring. Routes: `POST /standing-claims`, `GET /standing-claims/{claim_id}`, `POST /standing-claims/{claim_id}/{recognize,narrow,deny}` (`cdp/api/standing.py`); `POST /decisions/{registry_name}/{decision_id}/attested-challenges` accepts the same optional `standing_claim_id` (`cdp/api/decisions.py`). Tests: `tests/migration/test_migration_015_standing_and_recusal.py` (13 static + 2 Postgres smoke), `tests/standing/test_standing_claim_service.py` (21 cases, including the load-bearing proof that a pending, minimally sufficient claim with no determination yet permits the first Challenge, and that `recognized`/`narrowed` also permit while only `denied` blocks), `tests/standing/test_standing_claim_api.py` (14 cases). Confirmed passing in CI job `full-cdp-slice-tests`: run `31146632317`, PR #53 head commit `868f191161f928e1cc3c4896737d5853a1d9b2be`, 2026-08-07T04:11:29Z, conclusion `success`, alongside the full pre-existing suite (`pr-guard` and `full-cdp-slice-tests` both `pass`) with no regressions. This is not Recusal (unimplemented in its entirety), not any Standing type other than Constitutional Affected-Party, not automatic Breach Record generation on `denied` (RFC-CDP-072 remains E0), and not the enforcement-projection half of RFC-CDP-033 §12's two-layer persistence model -- see `docs/session-035-affected-party-standing-challenge.md` for the full boundary statement. |
